...
//
// PINNED: `Pagedraft.Api.Tests.ShippedRepairCorpusTests`, in `ShippedRepairCorpusTests.cs` at the TEST
// PROJECT ROOT, asserts this helper agrees with the shipped `PerType` map, in BOTH directions, so a future
// enable/rollback that fails to move the corpus turns RED instead of silently stranding it again.
// Deterministic: config file read plus static fixture lists, NO model, NO GPU, NO DB.
//
// WHY THE PIN IS NOT IN THIS FILE. It used to be, and that made it unreachable: this namespace,
// `Pagedraft.Api.Tests.LanguageEngine`, is the one the standing deterministic filter EXCLUDES
// (`FullyQualifiedName!~Pagedraft.Api.Tests.LanguageEngine`) because the folder holds the live-GPU
// harnesses, so the anti-stranding pin was itself stranded. The HELPERS stay here, next to their live
// consumer `OutputQualityDiagnostic`; only the `[Fact]`s moved out. Do not move them back.
// ---------------------------------------------------------------------------
